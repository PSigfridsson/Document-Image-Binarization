...
def buildfusionnet(inputs,numblock):
    ...
    for n in xrange(numblock):
		w = tf.compat.v1.get_variable('w'+str(n),[1],tf.float32,tf.compat.v1.constant_initializer(initw))
    ...
    fnet = tf.expand_dims(tf.reduce_mean(input_tensor=fnet,axis=3),axis=3)
    ...
    for n in xrange(numblock):
		...
		with tf.compat.v1.variable_scope('block'+str(n)):
			...
			#net = rnet
				...
				w = tf.compat.v1.get_variable('w'+str(n),[num_channels],tf.float32,tf.compat.v1.constant_initializer(1.0/num_channels))
				wnet = tf.multiply(rnet,w)
			
				wnet = tf.reduce_sum(input_tensor=wnet,axis=3,keepdims=True)
				netlist.append(wnet)
...
def buildfusionDenselynet(inputs,numblock):
    ...
    fnet = tf.reduce_mean(input_tensor=fnet,axis=3)
...
def denseResUnetBlock(inputs): #KLAR MAYBE
	
	#numFilter=[16,32,64,128,256]
	numFilter=[8,16,32,48,64]
	
	#256
	conv1_model = layers.Conv2d([3,3],numFilter[0],'conv1',activation_fn=leaky_relu)
	conv1 = conv1_model(inputs)
	pool1 = layers.max_pooling(conv1,[2,2],'pool1')
	
	#128
	#print('layer2 input shape:',pool1.get_shape())
	
	conv2_model = layers.Conv2d([3,3],numFilter[1],'conv2',activation_fn=leaky_relu)
	conv2 = conv2_model(pool1)
	...
	conv3_model = layers.Conv2d([3,3],numFilter[2],'conv3',activation_fn=leaky_relu)
	conv3 = conv3_model(pool13)
	...
	conv4_model = layers.Conv2d([3,3],numFilter[3],'conv4',activation_fn=leaky_relu)
	conv4 = conv4_model(pool14)
	...
	conv5_model = layers.Conv2d([3,3],numFilter[4],'conv5',activation_fn=leaky_relu)
	conv5 = conv5_model(pool15)
	pool5 = layers.max_pooling(conv5,[2,2],'pool5')
	# 8
	
	up_conv6_model = layers.Deconv_upsample(2,'upsample6',activation_fn=leaky_relu)
	up_conv6 = up_conv6_model(pool5)
	up_conv6 = layers.concat(up_conv6,conv5,'cont6')
	up_conv6_model = layers.Conv2d([3,3],numFilter[3],'conv6',activation_fn=leaky_relu)
	up_conv6 = up_conv6_model(up_conv6)
	
	
	up_conv7_model = layers.Deconv_upsample(2,'upsample7',activation_fn=leaky_relu)
	up_conv7 = up_conv7_model(up_conv6)
	up_conv7 = layers.concat(up_conv7,conv4,'cont7')
	up_conv7_model = layers.Conv2d([3,3],numFilter[2],'conv7',activation_fn=leaky_relu)
	up_conv7 = up_conv7_model(up_conv7)
	
	up_conv8_model = layers.Deconv_upsample(2,'upsample8',activation_fn=leaky_relu)
	up_conv8 = up_conv8_model(up_conv7)
	up_conv68_model = layers.Deconv_upsample(4,'upsample68',activation_fn=leaky_relu)
	up_conv68 = up_conv68_model(up_conv6)
	
	up_conv8 = layers.concat(up_conv8,conv3,'cont8')
	up_conv8 = layers.concat(up_conv8,up_conv68,'cont68')
	
	up_conv8_model = layers.Conv2d([3,3],numFilter[1],'conv8',activation_fn=leaky_relu)
	up_conv8 = up_conv8_model(up_conv8)
	
	up_conv9_model = layers.Deconv_upsample(2,'upsample9',activation_fn=leaky_relu)
	up_conv9 = up_conv9_model(up_conv8)
	up_conv69_model = layers.Deconv_upsample(8,'upsample69',activation_fn=leaky_relu)
	up_conv69 = up_conv69_model(up_conv6)
	up_conv79_model = layers.Deconv_upsample(4,'upsample79',activation_fn=leaky_relu)
	up_conv79 = up_conv79_model(up_conv7)
	...
	up_conv9_model = layers.Conv2d([3,3],numFilter[0],'conv9',activation_fn=leaky_relu)
	up_conv9 = up_conv9_model(up_conv9)
	
	
	up_conv10_model = layers.Deconv_upsample(2,'upsample10',activation_fn=leaky_relu)
	up_conv10 = up_conv10_model(up_conv9)
	up_conv610_model = layers.Deconv_upsample(16,'upsample610',activation_fn=leaky_relu)
	up_conv610 = upconv610_model(up_conv6)
	up_conv710_model = layers.Deconv_upsample(8,'upsample710',activation_fn=leaky_relu)
	up_conv710 = upconv710_model(up_conv7)
	up_conv810_model = layers.Deconv_upsample(4,'upsample810',activation_fn=leaky_relu)
	upconv810 = upconv810_model(upconv8)
	...
	outlayer_model = layers.Conv2d([3,3],1,'conv10',activation_fn=leaky_relu)
	outlayer = outlayer_model(up_conv10)
...
def resHEDUNetBlock(inputs,nlayers=1,shortCut=False):
	...
	if shortCut == True:
		...
		weights = tf.compat.v1.get_variable('side_weight',[nSide],tf.float32,tf.compat.v1.constant_initializer(1.0/nSide))
		
		for n in xrange(nSide):
			if n == 0:
